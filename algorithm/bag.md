...
- 一共有4种求解目标:
  ...
  - 求组合数
  - 求排列数
  - 给`target`,求最少/最多的物品数量

> 其中01背包不考虑排列数,完全背包不考给出target的最多物品数量
...
## 题型6:01背包 + 最少/最多物品数量
```py
def knap01_max_len(nums, target):
    dp = [-float('inf')] * (target + 1)         # 初始化为负无穷
    dp[0] = 0
    for num in nums:                            # 物品外循环
        for j in range(target, num - 1, -1):    # 背包倒序
...
def knap01_min_len(nums, target):
    dp = [float('inf')] * (target + 1)          # 初始化为正无穷
    dp[0] = 0
    for num in nums:                            # 物品外循环
        for j in range(target, num - 1, -1):    # 背包倒序
...
## 题型7:完全背包 + 最少物品数量
```py
def complete_min_len(coins, amount):
    dp = [float('inf')] * (amount + 1)          # 初始化为正无穷
    dp[0] = 0
    for coin in coins:                          # 物品外循环
        for j in range(coin, amount + 1):       # 背包正序
