...
## 现代工程配置基座：version "2" 架构解构

`golangci-lint` 在现代版本中推出了 `version: "2"` 配置架构，强化了 JSON Schema 规范校验、预设分组（Presets）与更精准的检查器分类。以下是兼顾**严苛质量门禁**与**工程实用性**的标准基座配置：
...
以下针对配置中启用的 9 项关键规则，逐一展现**无静态规则约束时的典型缺陷代码**与**规则门禁严管下的防御性标准实现**。

> [!TIP] 视图联动
> 点击任意代码块顶部的 Tab 标签（「无规则拦截 (典型缺陷代码)」或「规则门禁约束 (标准实现)」），全页所有规则的比对视图将同步切换。
